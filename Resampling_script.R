...
singleTreeSize <- function(size,
                           ...
                           aln_number = 100,
                           aln_length =  1000,
                           aln_gamma4C = NULL, #Discrete Gamma model with n categories
                           aln_I = NULL, #A proportion of invariable sites
                           aln_indel = NULL, #"0.03,0.1" <INS_DIS>,<DEL_DIS> Set the insertion and deletion size distributions
                           aln_indel_size = NULL #<INS_DIS>,<DEL_DIS> Set the insertion and deletion size distributions
                           ){
  ...
  system(paste0("iqtree2 --alisim ", aln_prefix, " -m ", aln_model,
                ...
                if(!is.null(aln_indel_size)){paste0(" --indel ",aln_indel_size)},
                " --no-unaligned"
  ))
...
params <- expand.grid(model = c("HKY","JC", "SYM", "GTR"),
                      aln_length = c(100, 500, 1000, 10000, 100000),
                      size = c(3, 4, 5, 10, 100, 1000, 10000)
            )
